...
function changePassword(): Partial<User> | null {
  ...
  const oldPasswordInput = (document.getElementById("old-password-input") as HTMLInputElement).value;
  const newPasswordInput = (document.getElementById("new-password-input") as HTMLInputElement).value;
  const repeatNewPasswordInput = (document.getElementById("repeat-new-password-input") as HTMLInputElement).value;
...
async function editDogInfo(target: HTMLElement) {
  ...
  const editedDog = getDogEdits();
   const dynamicContent = `
      <h2>Lagrer endringer...</h2>
          <div class="dog-spinner" alt="Loading spinner" draggable="false"/></div>
      `;
   createModal(dynamicContent);
